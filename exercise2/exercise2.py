...
#   the size of it is 20025,the shape of it is (25, 801).
...
x_rec = np.sum(x_rec,axis=0) #add the elements of each column
...
winlen=512
b = np.array([1,-1]) #b contain the coefficients
w_,h_=freqz(b,1,int(winlen/2)+1)
plt.figure(2)
plt.plot(w_, 20*np.log10(np.abs(h_)))
plt.xlabel("The normalized frequencies (Hz)")
plt.ylabel("The log-scaled frequency response (dB)")
plt.show()
...
print("window length is " + str(float(1000*winlen/fs)) + ' ms ') 
...
#yes, it increases. as is shown in the time domain, the size is 20009, and in the STFT domain, it is 20224.
...
_,x_rec_filt = istft(H*X,fs=fs,window=win,nperseg=winlen,noverlap=winlen/2,nfft=winlen,input_onesided=True)
...
#in the fisrt figure, it is perfectly reconstructed and in the second one, it is not that perfect. the error is 
#1.04922259637e-09.