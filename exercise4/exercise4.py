...
#7SMR = Signal level – min( subband masking threshold )76-70=6
...
#The uniform quantization steps: 2^6 = 64
#The step-size: 1/64
...
# The threshold characterizes the amount of energy needed in a pure tone such that it can b
...
f = np.linspace(1, fs/2, int(winlen/2.0+1), endpoint=True)
...
# Normalized_LEVEL = 96 - 20*np.log10( np.abs( X_DFT ) / level_at_1khz )
...
fv = np.linspace(0,fs/2,int(winlen/2.0+1))
